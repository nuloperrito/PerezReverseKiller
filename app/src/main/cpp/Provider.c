...
int InitProvider(JNIEnv *env) {
	...
	if(TestProvider == NULL) {
		TestProvider = (*env)->FindClass(env,"com/perez/netdiag/Service/LDNetTraceRoute");
		if(TestProvider == NULL){
			return -1;
		}
	...
	if (mTestProvider == NULL) {
		jfieldID fid = (*env)->GetStaticFieldID(env, TestProvider, "instance", "Lcom/perez/netdiag/Service/LDNetTraceRoute;");
		if(fid == NULL){
			__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "InitProvider Begin get static object id error" );
			return -2;
		}

		mTestProvider = (*env)->GetStaticObjectField(env, TestProvider, fid);
		if (mTestProvider == NULL) {
	...
	if (printTraceInfo == NULL) {
		printTraceInfo = (*env)->GetMethodID(env, TestProvider, "printTraceInfo","(Ljava/lang/String;)V");
		if (printTraceInfo == NULL) {
			(*env)->DeleteLocalRef(env, TestProvider);
			(*env)->DeleteLocalRef(env, mTestProvider);
			return -2;
		}
...
int InitSocketInfo(JNIEnv* jniEnvTelnet) {
	...
	if(_LDNetSocket == NULL) {
		_LDNetSocket = (*jniEnvTelnet)->FindClass(jniEnvTelnet,"com/perez/netdiag/Service/LDNetSocket");
		if(_LDNetSocket == NULL){
			return -1;
		}
		__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "InitProvider Begin  2 ok" );
	...
	if (_mLDNetSocket == NULL) {
		jfieldID fid = (*jniEnvTelnet)->GetStaticFieldID(jniEnvTelnet, _LDNetSocket, "instance", "Lcom/perez/netdiag/Service/LDNetSocket;");
		if(fid == NULL){
			__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "InitProvider Begin get static object id error" );
			return -2;
		}
