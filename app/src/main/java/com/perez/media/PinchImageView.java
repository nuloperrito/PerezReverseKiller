...
public class PinchImageView extends AppCompatImageView {
    ...
    public float[] getImageCenterNormalized() {
        if (!isReady()) return new float[]{0.5f, 0.5f};
        Matrix matrix = MathUtils.matrixTake();
        getCurrentImageMatrix(matrix);
        float[] center = MathUtils.inverseMatrixPoint(new float[]{getWidth() / 2f, getHeight() / 2f}, matrix);
        MathUtils.matrixGiven(matrix);
        return new float[]{
                center[0] / getDrawable().getIntrinsicWidth(),
                center[1] / getDrawable().getIntrinsicHeight()
        };
    }

    public float getOuterScale() {
        return MathUtils.getMatrixScale(mOuterMatrix)[0];
    }

    public void restoreScaleAndCenter(float normX, float normY, float outerScale) {
        if (!isReady()) return;

        int imgW = getDrawable().getIntrinsicWidth();
        int imgH = getDrawable().getIntrinsicHeight();
        float targetX = normX * imgW;
        float targetY = normY * imgH;

        Matrix innerMatrix = MathUtils.matrixTake();
        getInnerMatrix(innerMatrix);
        float innerScale = MathUtils.getMatrixScale(innerMatrix)[0];

        if (outerScale < 1f) outerScale = 1f;
        if (outerScale * innerScale > getMaxScale()) {
            outerScale = getMaxScale() / innerScale;
        }

        float[] innerPt = new float[2];
        innerMatrix.mapPoints(innerPt, new float[]{targetX, targetY});
        MathUtils.matrixGiven(innerMatrix);

        float dx = getWidth() / 2f - innerPt[0] * outerScale;
        float dy = getHeight() / 2f - innerPt[1] * outerScale;

        mOuterMatrix.reset();
        mOuterMatrix.postScale(outerScale, outerScale);
        mOuterMatrix.postTranslate(dx, dy);

        clampBoundsImmediate();
        dispatchOuterMatrixChanged();
        invalidate();
    }

    public void clampBoundsImmediate() {
        if (!isReady()) return;
        ...
        if (bound.right - bound.left < displayWidth) {
            postX = displayWidth / 2f - (bound.right + bound.left) / 2f;
        ...
        if (bound.bottom - bound.top < displayHeight) {
            postY = displayHeight / 2f - (bound.bottom + bound.top) / 2f;
        } else if (bound.top > 0) {
            postY = -bound.top;
        ...
        if (postX != 0 || postY != 0) {
            mOuterMatrix.postTranslate(postX, postY);
        }
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (oldw > 0 && oldh > 0 && (w != oldw || h != oldh) && isReady()) {
            float[] oldCenter = MathUtils.inverseMatrixPoint(new float[]{oldw / 2f, oldh / 2f}, getCurrentImageMatrix(null));
            float normX = oldCenter[0] / getDrawable().getIntrinsicWidth();
            float normY = oldCenter[1] / getDrawable().getIntrinsicHeight();
            float outerScale = getOuterScale();
            if (outerScale > 1.001f) {
                restoreScaleAndCenter(normX, normY, outerScale);
            } else {
                reset();
            }
        }
